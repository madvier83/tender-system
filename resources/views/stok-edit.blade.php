    ...
    <x-drawer>
        ...
        <div class="p-8">
            ...
            <div class="mt-8 h-max rounded-xl max-w-lg">
                <form action="/stok/{{ $stok->id }}" method="POST" enctype="multipart/form-data">
                    @method('PUT')
                    @csrf
                        ...
                        <select name="barang_id" id="barang_id" class="selectBarang">
                            @foreach ($barangs as $barang)
                                <option value="{{ $barang->id }}"
                                    {{ (old('barang_id') ?? $stok->barang_id) == $barang->id ? 'selected' : '' }}>
                                    {{ $barang->nama }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <label class="form-control w-full mt-2">
                        ...
                        <select name="tipe" class="input input-bordered w-full">
                            <option value="masuk" {{ (old('tipe') ?? $stok->tipe) == 'masuk' ? 'selected' : '' }}>
                                Barang Masuk</option>
                            <option value="keluar" {{ (old('tipe') ?? $stok->tipe) == 'keluar' ? 'selected' : '' }}>
                    ...
                    <label class="form-control w-full mt-2">
                        ...
                        <input name="kuantitas" type="text" placeholder="Tulis disini"
                            class="input input-bordered w-full" value="{{ old('kuantitas') ?? $stok->kuantitas }}" />
                        @error('kuantitas')
                            <p class="text-sm text-rose-400 mt-2">{{ $message }}</p>
                        @enderror
                    </label>
                    ...
                    <label class="form-control w-full mt-2">
                        ...
                        <input name="stok_awal" type="text" placeholder="Tulis disini"
                            class="input input-bordered w-full" value="{{ old('stok_awal') ?? $stok->stok_awal }}" />
                        @error('stok_awal')
                            <p class="text-sm text-rose-400 mt-2">{{ $message }}</p>
                        @enderror
                    </label>
                    ...
                    <label class="form-control w-full mt-2">
                        ...
                        <input name="stok_setelah_exp" type="text" placeholder="Tulis disini"
                            ...
                            value="{{ old('stok_setelah_exp') ?? $stok->stok_setelah_exp }}" />
                    ...
                    <label class="form-control w-full mt-2">
                        ...
                        <input name="tanggal" type="date" placeholder="Tulis disini"
                            class="input input-bordered w-full" value="{{ old('tanggal') ?? $stok->tanggal }}" />
                        @error('tanggal')
                            <p class="text-sm text-rose-400 mt-2">{{ $message }}</p>
                        @enderror
                    </label>
                    ...
                    <label class="form-control w-full mt-2">
                        ...
                        <input name="tanggal_exp" type="date" placeholder="Tulis disini"
                            class="input input-bordered w-full"
                            value="{{ old('tanggal_exp') ?? $stok->tanggal_exp }}" />
                        @error('tanggal_exp')
                            <p class="text-sm text-rose-400 mt-2">{{ $message }}</p>
                        @enderror
                    </label>
